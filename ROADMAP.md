...
- [x] **G3.6 — per-track `amplify` annotation + annotation rename**: the
      ...
      the note in the backlog). `amplify` accepts a linear factor or a dB
      value (`"-8.2 dB"`) and lands in `TrackCues`, applied by a
      `TrackGainSource` wrap in `apply_cues` (outermost, so it stacks
      with cue cuts and reaches the mixer transparently). Acceptance met:
      parse tests for linear/dB/malformed values, unit tests on the gain
      wrapper, and a script-level test where `amplify="0.0"` mutes a
      loud jingle that audibly plays unannotated.
...
- [x] **D2 — per-track cue points (`annotate:` prefix + `cue_cut`)**: parse
      `annotate:cue_in="30",cue_out="180":/path/track.mp3` in
...
- [ ] **G3.6 — per-track `amplify` annotation.** Crabsoup's own annotation
      ...
      `amplify(src, gain)` operator exists; the annotation is missing.
      Parse it into `TrackCues` alongside `fade_in/fade_out` and apply the
      gain where replaygain applies its per-track gain (the same code
      path, so no new mixer logic). Acceptance:
      `annotate:amplify="0.5":...` halves the track's level (peak
...
- **Annotation vocabulary note (2026):** crabsoup's `annotate:` keys are
  ...
  The `annotate:` prefix is already a dedicated namespace, so no `liq_`
  disambiguation is needed, and the keys match the Lua operator options
  (`cue_cut(src, {cue_in, fade_out})`). URIs generated by Liquidsoap
  tooling keep playing (unknown keys are ignored) but lose their cue
  points/fades until rewritten.
...
- [x] D2 step 2 (per-track crossfade override): `AudioSource::
      ...
      so `annotate:fade_in=...` alone reaches the mixer. Inline tests:
      ...
      `single("annotate:fade_in=...,fade_out=...:path")` surfaces
...
- [x] D2 step 1 (`annotate:`/`cue_cut`): `src/request.rs` — `RequestUri`
      ...
      a crabsoup `annotate:` prefix (`cue_in`, `cue_out`,
      `fade_in`, `fade_out`; unknown keys ignored; malformed
