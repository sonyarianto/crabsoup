...
- [x] Jingles: `jingles:` YAML key parsed (`directory` + `files`), playable via a
      new Liquidsoap-style telnet control port (`jingles.list`, `jingles.play
      [n|substr]`, `shutdown`). Verified end-to-end with the 2 staged jingles.
...
- [ ] Preview mode via `--preview`? (currently only by omitting `output:` in YAML)
